...
mutable struct FINITO_LFinito_batch_state{R<:Real,Tx}  
...
function FINITO_LFinito_batch_state(γ::Array{R}, hat_γ::R, av::Tx, ind, d) where {R,Tx}
    return FINITO_LFinito_batch_state{R,Tx}(
        γ,
        hat_γ,
        av,
        ind,
...
function Base.iterate(iter::FINITO_LFinito_batch_iterable{R,Tx}) where {R,Tx} 
    N = iter.N
...
function Base.iterate(
    iter::FINITO_LFinito_batch_iterable{R,Tx},
    state::FINITO_LFinito_batch_state{R,Tx},
) where {R,Tx}
    # full update 
